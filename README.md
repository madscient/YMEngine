...
OPM は fnum ではなく **KC (Key Code) + KF (Key Fraction)** 方式です。

```
0x28+ch : KC  — bit6-4=OCT(0-7), bit3-0=NOTE
0x30+ch : KF  — bit7-2=Key Fraction (6bit, 0-63)
```

**NOTE の並びは半音階そのままではなく、C# 始まり・C 終わりの特殊な配置です** (0,1,2,4,5,6,8,9,10,12,13,14 の12値のみ有効。3,7,11,15 は欠番):

| NOTE | 0 | 1 | 2 | 4 | 5 | 6 | 8 | 9 | 10 | 12 | 13 | 14 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 音名 | C# | D | D# | E | F | F# | G | G# | A | A# | B | C |

同一 OCT 内では NOTE=0(C#) が最低音、NOTE=14(C) が最高音になります。OCT=n, NOTE=14(C) はほぼ OCT=n+1 の C に相当する音です。

**KC は、マスタークロックが 3.579545 MHz のとき KF=0 で平均律の周波数になるよう設計されています。** マスタークロックがこれ以外の場合は音程がずれるため、**KF で補正する必要があります**（KC をズラして帳尻を合わせるのは誤り）。

ずれの大きさは次式で計算できます (fM=3.579545MHz が基準):

```
ずれ[セント] = log(fMX / fM) / log(2) × 1200
```

例えば fMX=4MHz の場合、約 +192.27 セント (≈1音) 音程が高くなります。これを補正するには KF に 0 ではなく「約 200 セント分」に相当する値 (NOTE によって異なるが概ね 5〜6、6bit 値としては `5<<2`〜`6<<2`) を与え、かつ NOTE を1段下げて使います。詳細な対応は YM2151 アプリケーションマニュアル「1-7 システムクロックが4MHzの際の補正方法」を参照してください。

YMEngine の標準クロックは `FmClock::OPM = 3'579'545` Hz (`FmChip.h`) のため、**KF=0 のまま NOTE テーブル通りの KC を使えば正確な平均律になります**。クロックを変更して使う場合のみ KF による補正を検討してください。
